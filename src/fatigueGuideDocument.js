    ...
    sections: [
      {
        ...
        points: [
          ...
          '1行目: 帯域・スコア・未完了の時刻あり予定件数。2行目: 睡眠・予定の内訳点数（各最大50）。',
          '予定の負荷は<strong>未完了の時刻あり予定のみ</strong>を数えます（タスクは含みません）。',
          '達成（連続日数・週バッジ）は選択日の<strong>予定カードの直下</strong>に表示され、タスクも含みます。',
      ...
      },
      {
        ...
        points: [
          ...
          '朝の間隔：起床時刻から最初の未完了の時刻あり予定開始まで30分未満の場合、最大5点を追加。',
        ],
      },
      {
        heading: '4. 予定側（最大50点）',
        body: '選択日の未完了の時刻あり予定だけを対象に、占有時間・件数・重要度・連続ブロック・夜間（21時以降）などから加点します。タスク（isTask）は対象外です。',
        points: [
          '占有時間：未完了の時刻あり予定の開始〜終了の合計が長いほど加点（最大20点）。',
          '件数：未完了の時刻あり予定が多いほど加点（最大10点）。',
          '重要：重要度「重要」の件数に応じて加点（最大12点）。',
          `連続：予定の間隔が${fatigueScoringReference.minGapMinutes}分未満は同じブロックとみなし、長い連続時間で加点（最大10点）。`,
          `夜間：${fatigueScoringReference.eveningStartHour}時以降に始まる未完了の時刻あり予定の時間で加点（最大8点）。`,
        ],
      },
      {
        heading: '5. 免責と説明PDF',
